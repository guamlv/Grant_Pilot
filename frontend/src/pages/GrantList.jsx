...
import { getGrants, createGrant, getSettings, portfolioAnalysis, grantDiscovery, parseDocument, createTask } from '@/services/api';
import { Plus, Sparkles, TrendingUp, Compass, ChevronDown, ChevronUp, Search, Filter, Loader2, Upload, Info } from 'lucide-react';
...
export const GrantList = () => {
  ...
  const [newG, setNewG] = useState({ title: '', status: 'Prospect', award_amount: 0, funder: '', deadline: '', description: '', probability: 50 });
  const [newTask, setNewTask] = useState({ description: '', due_date: '' });

  useEffect(() => {
    getGrants().then(res => setGrants(res.data || [])).catch(() => {});
  ...
  const saveGrant = async () => {
    if (newG.title) {
        const res = await createGrant({ 
          ...newG, 
          start_date: newG.start_date || new Date().toISOString().split('T')[0]
        });
        setGrants([...grants, res.data]);
        setModal(null);
        setNewG({ title: '', status: 'Prospect', award_amount: 0, funder: '', deadline: '', description: '', probability: 50 });
  ...
  const saveTask = async () => {
      if (modal?.id && newTask.description) {
          await createTask({ grant_id: modal.id, ...newTask, is_completed: false });
          setModal(null);
  ...
  const runAnalysis = async () => {
      setLoading(true);
      try {
          const summary = grants.map(g => `${g.title} (${g.status}): $${g.award_amount}`).join('; ');
          const res = await portfolioAnalysis({ grants_count: grants.length, grants_summary: summary });
          setAnalysis(res.data);
      } catch (e) {
        addToast({ type: 'error', title: 'Analysis Failed', message: 'Could not generate portfolio analysis.' });
  ...
  const runDiscovery = async () => {
      setLoading(true);
      try {
          const res = await grantDiscovery({ 
            organization_name: settings?.organization_name || 'Organization',
            current_focus: grants.map(g => g.title).slice(0, 3).join(', ')
          });
          setRecs(res.data);
          setModal({ type: 'disc' });
      } catch (e) {
        addToast({ type: 'error', title: 'Discovery Failed', message: 'Could not find grant recommendations.' });
      } finally { setLoading(false); }
  };
  ...
  const handleFileUpload = async (e) => {
      ...
      try {
          ...
          reader.onload = async () => {
              ...
              try {
                const res = await parseDocument({ base64_data: base64, mime_type: file.type });
                const data = res.data;
                setNewG(prev => ({ 
                  ...prev, 
                  title: data.title || prev.title,
                  funder: data.funder || prev.funder,
                  award_amount: data.awardAmount || data.award_amount || prev.award_amount,
  ...
  return (
    <div className="space-y-12 pb-32 max-w-7xl mx-auto" data-testid="grant-list">
       <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
           ...
           <div className="flex gap-4 flex-wrap">
               <button onClick={runAnalysis} disabled={loading || grants.length === 0} className="px-6 py-4 bg-white border border-zinc-200 text-black rounded-[1.5rem] text-sm font-black hover:bg-zinc-50 flex items-center gap-3 transition-all shadow-sm disabled:opacity-50" data-testid="portfolio-ai-btn">
               ...
               <button onClick={runDiscovery} disabled={loading} className="px-6 py-4 bg-white border border-zinc-200 text-black rounded-[1.5rem] text-sm font-black hover:bg-zinc-50 flex items-center gap-3 transition-all shadow-sm disabled:opacity-50" data-testid="discover-btn">
                   {loading && !recs ? <Loader2 className="animate-spin" size={18}/> : <Compass size={18} className="text-zinc-400"/>} <span>Discover</span>
               </button>
               <button onClick={() => setModal({type:'new'})} className="px-8 py-4 bg-black text-white rounded-[2rem] text-sm font-black hover:scale-105 transition-all shadow-2xl flex items-center gap-3" data-testid="create-grant-btn">
           ...
           <div className="bg-black text-white p-10 rounded-[3.5rem] animate-in slide-in-from-top-6 shadow-2xl border border-zinc-800">
               ...
               <div className="grid md:grid-cols-2 gap-16">
                   <div className="space-y-6">
                       ...
                       <ul className="space-y-4 text-zinc-300 text-lg font-medium leading-relaxed">
                         {analysis.trends?.map((t, i) => <li key={i} className="flex gap-4"><span className="text-emerald-500">/</span> {t}</li>)}
                       </ul>
                   </div>
                   <div className="space-y-6">
                       ...
                       <ul className="space-y-4 text-zinc-300 text-lg font-medium leading-relaxed">
                         {analysis.strategy?.map((s, i) => <li key={i} className="flex gap-4"><span className="text-emerald-500">/</span> {s}</li>)}
                       </ul>
                   </div>
               </div>
           </div>
       ...
       <div className="bg-white border border-zinc-200 rounded-[3.5rem] overflow-hidden shadow-2xl shadow-zinc-200/50">
           <div className="p-8 border-b border-zinc-100 flex flex-col md:flex-row gap-6 bg-zinc-50/30">
               <div className="relative flex-1">
                   ...
                   <input className="w-full pl-14 pr-6 py-5 rounded-3xl border border-zinc-100 focus:outline-none focus:ring-2 focus:ring-black/5 bg-white font-bold" placeholder="Search funding vault..." value={search} onChange={e => setSearch(e.target.value)} data-testid="search-input" />
               </div>
               <div className="relative">
                   <Filter className="absolute left-6 top-1/2 -translate-y-1/2 text-zinc-400" size={20}/>
                   <select className="pl-14 pr-12 py-5 rounded-3xl border border-zinc-100 appearance-none bg-white focus:outline-none font-bold min-w-[180px]" value={filter} onChange={e => setFilter(e.target.value)} data-testid="filter-select">
           ...
           <div className="divide-y divide-zinc-50">
                   ...
                   <div key={g.id} className="group hover:bg-zinc-50/50 transition-all duration-300">
                       <div className="p-8 flex items-center gap-8 cursor-pointer" onClick={() => toggleExpand(g.id)}>
                           ...
                           <div className="flex-1 min-w-0">
                               ...
                               <p className="text-base text-zinc-500 font-medium mt-1">{g.funder} • <span className="text-black font-black">{formatCurrency(g.award_amount)}</span></p>
                           </div>
               ...
               {filtered.length === 0 && <div className="p-32 text-center text-zinc-400 font-bold uppercase tracking-[0.2em] text-sm">No records found.</div>}
           </div>
       </div>
       ...
       <Modal isOpen={!!modal} onClose={() => setModal(null)} title={modal?.type === 'new' ? 'Initialize Grant' : modal?.type === 'task' ? 'Add Quick Task' : 'Intelligent Recommendations'}>
           {modal?.type === 'new' && (
               <div className="space-y-8">
                   ...
                   <button onClick={saveGrant} className="w-full bg-black text-white py-5 rounded-[2rem] font-black text-sm uppercase tracking-widest hover:scale-[1.02] active:scale-95 transition-all shadow-2xl" data-testid="save-grant-btn">Confirm Initialization</button>
               </div>
           )}
           {modal?.type === 'task' && (
               <div className="space-y-6">
                   ...
                   {(Array.isArray(recs) ? recs : []).map((r, i) => (
                       <div key={i} className="bg-zinc-50 p-8 rounded-[2rem] border border-zinc-100 hover:shadow-xl transition-all group">
                           <div className="flex justify-between items-start mb-6">
                               ...
                               <span className="bg-black text-white text-[10px] font-black uppercase tracking-widest px-4 py-1.5 rounded-full">{r.matchScore}% Match</span>
                           </div>
                           <p className="text-zinc-600 text-sm font-medium leading-relaxed mb-6">{r.rationale}</p>
                           <div className="flex flex-wrap gap-2">
                             {r.suggestedFunders?.map((f, j) => <span key={j} className="text-[10px] font-black uppercase tracking-widest text-zinc-400 border border-zinc-200 px-3 py-1 rounded-lg">{f}</span>)}
                           </div>
                       </div>
                   ))}
               </div>
