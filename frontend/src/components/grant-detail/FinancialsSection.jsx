...
export const FinancialsSection = ({ grant }) => {
  ...
  useEffect(() => {
    if (grant?.id) {
      getFinancials(grant.id).then(res => setRawFin(res.data)).catch(() => {});
    }
  }, [grant?.id]);
  
  const fins = useMemo(() => {
    if (!rawFin || rawFin.length === 0) return [];
    return rawFin
        .sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())
        .map(s => calculateFinancialMetrics(s, grant.award_amount));
  }, [grant, rawFin]);
